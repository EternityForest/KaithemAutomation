...
# Compose files. The override is auto-generated by scripts/render-compose-override.sh
# so that group_add becomes a real YAML list (compose env var interpolation is
# text-only and cannot split `KAITHEM_GROUPS` into a sequence).
COMPOSE_FILE_BASE := docker/docker-compose.yaml
COMPOSE_FILE := $(COMPOSE_FILE_BASE)
IN_DEV_DOCKER := docker compose -f $(COMPOSE_FILE_BASE) up -d --no-build kaithem-dev && docker compose -f $(COMPOSE_FILE_BASE) exec kaithem-dev
IN_APP_DOCKER := docker compose -f $(COMPOSE_FILE_BASE) up -d --no-build kaithem && docker compose -f $(COMPOSE_FILE_BASE) exec kaithem
PLAYWRIGHT := docker compose -f $(COMPOSE_FILE_BASE) run --rm playwright npx playwright
...
dev-record-playwright: # Record playwright tests
	@bash scripts/render-compose-override.sh
...
dev-run-all-tests: export IMAGE_PREFIX=localhost:5000/
...
# Tests should run on our locally built copy
dev-run-playwright-against-production-app: export IMAGE_PREFIX=localhost:5000/

.PHONY: dev-run-playwright-against-production-app
...
.PHONY: dev-create-producton-buildx-context
...
dev-build-docker-production:
	...
	@BUILDX_BUILDER=kaithem-multiarch-builder docker buildx bake -f docker-compose.yaml -f docker-bake.hcl --progress=plain
	...
	@skopeo copy --multi-arch all oci-archive:${DOCKER_BUILD_DIR}/kaithem-dev-${KAITHEM_VERSION}.tar docker://localhost:5000/cooperskeep/kaithem-dev:${KAITHEM_VERSION} --dest-tls-verify=false
# Pull those images locally
	@docker pull localhost:5000/cooperskeep/kaithem:${KAITHEM_VERSION}
...
.PHONY: dev-docker-print-compose
dev-docker-print-compose:
	@cd ./docker
	@docker compose config

.PHONY: dev-docker-print-bake
dev-docker-print-bake:
	@cd ./docker
	@docker buildx bake --print -f docker-compose.yaml -f docker-bake.hcl

dev-docker-kiosk: export IMAGE_PREFIX=localhost:5000/

.PHONY: dev-docker-kiosk
dev-docker-kiosk: # Launch the kiosk browser in a docker.
	@bash scripts/render-compose-override.sh 
	@cd ./docker
	@KIOSK_URL=http://localhost:8002 docker compose -f docker-compose.yaml -f docker-compose.override.yaml run --remove-orphans kaithem-kiosk

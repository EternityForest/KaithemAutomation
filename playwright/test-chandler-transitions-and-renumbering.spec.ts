...
test("test", async ({ page }) => {
  ...
  await chandlerBoardTemplate(page, module);
    ...
    await page.getByRole("row", { name: "c2" }).getByTitle("Cue Length").fill("5");
  ...
  await page.getByText("6", { exact: true }).dblclick();
    ...
    // Give it a five second fade
  await page
    .getByRole("row", { name: "15 c3" })
...
// Ensure the tag point is actually fading in slowly
  ...
  expect(Number.parseFloat(value2)).toBeGreaterThan(0);
    ...
    await page.getByTestId('extras-button-playwrightchandlertestmodule2_board1').click();
    await page.getByRole('link', { name: 'Editor' }).click();

    await page.getByRole('button', { name: 'tst1' }).click();

    // Set c3 to end in 0.25s
    await page.getByRole("row", { name: "c3" }).getByRole('combobox', { name: 'Cue Length' }).fill('0.25');


    await page.getByText('tst1 (running)Page').click();
    // Go on c3 again
    ...
    await page.getByRole('button', { name: 'tst1' }).click();
    await page.getByRole('row', { name: 'c3' }).locator('select').selectOption('default');
    await page.getByTestId('close-group').click();
  
    // It immediately goes to default
    await expect(page.getByTestId('sidebar-active-cue-name')).toContainText('default');
    ...
    await page.getByRole("row", { name: "c2" }).getByTitle("Cue Length").fill("0");
    ...
    await sleep(50);
    // Go on c4
    page.getByRole("row", { name: "c4" }).getByRole('button', { name: 'Go', exact: true }).click();
    await sleep(50);

...
test("test", async ({ page }) => {
  ...
  await login(page);
    ...
    await page.getByTestId('dashbeard-blurb-testboard').getByTestId('edit-db-map').click();
    ...
    await page.getByRole('combobox', { name: 'Tag' }).fill('/chandler/groups/testgroup1.alpha');
    await expect(page.getByRole('textbox', { name: 'Enter value' })).toHaveValue('0.34');
    ...
    await page.locator('summary').filter({ hasText: 'Bindings' }).click();
    ...
    await expect(page.locator('.small-dashboard-widget-container')).not.toBeVisible();
    ...
    await page.locator('div').filter({ hasText: 'Edit Commander Config' }).first().click();
    ...
    await expect(page.getByRole('slider', { name: 'Value (1.00)' })).toHaveValue('1');

...
class ChannelStrip(gstwrapper.Pipeline, BaseChannel):
    ...
    def mpv_input_loop(self):
        ...
        x = jackmanager.Airwire(
            n, self.name + "_in", forceCombining=(self.channels == 1)
        )
        x.connect()
                ...
                if param == "*destination":
                    ...
                    if effectId in self.sendAirwires:
                        ...
                        self.sendAirwires[effectId] = jackmanager.Airwire(
                            ...
                            value,
                            forceCombining=(self.channels == 1),
                        )
                        try:
    ...
    def addLevelDetector(self):
        self.levelDetector = self.addElement(
            "level",
            post_messages=True,
    ...
    def _createChannel(self, name, data=channelTemplate):
        ...
        if not name in self.channelAlerts:
            self.channelAlerts[name] = alerts.Alert(
                "Mixer channel " + name, priority="error", tripDelay=35, autoAck=True
            )

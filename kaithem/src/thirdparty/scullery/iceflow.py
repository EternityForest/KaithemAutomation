...
class GStreamerPipeline():
    ...
    def seek(self, t=None, rate=None, _raw=False, _offset=0.008, flush=True, segment=False):
        ...
        with self.lock:
            ...
            def f():
                ...
                with self.seeklock:
                    if (not flush) and self.pipeline.get_state(1000_000_000)[1] == Gst.State.PAUSED:
                        raise RuntimeError("Cannot do non-flushing seek in paused state as this may deadlock.  State has changed while request inflight, request cancelled.")
    ...
    def shouldAllowGstJack():
        raise RuntimeError(
            "You must override this method to return True if jackd is running and can be used")

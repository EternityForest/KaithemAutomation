...
class DNSCache:
    ...
    def add(self, entry):
        ...
        #If there's just way too many entries here, remove a random one.
        #Really this should never be needed, it's just so it doesn't fill
        #RAM and crash if something really odd happens.
        
        #We allow up to 2048 keys with 4096 entries each.
        #Probably too many still, but at least we're trying something.
        #This modification by Daniel Dunn.
        try:
            if len(self.cache)>2048:
                self.cache.pop(random.choice(list(self.cache.keys())))
        except:
            logging.exception("Error removing old cache entry")
            
        try:
            if entry.key in self.cache and len(self.cache[entry.key])> 4096:
                x = random.choice(self.cache[entry.key])
                self.remove(x)
        except:
            logging.exception("Error removing random old cache entry")

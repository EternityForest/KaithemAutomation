...
def refreshAutoGeneratedSoundAliases(t=None,v=None):


    #Don't clog everything up with this for every sound effect
    if v and v.name.startswith("kplayer"):
        return
    ...
    if not commonSoundAliasesl:
        ...
        # kaithem handle jack
        try:
            x = jackmanager.getPersistentCardNames()
            for i in x:
                p = SoundDeviceAlias()
                p.alsaName = x[i][0]
                commonSoundAliasesl[i] = p
        except:
            log.exception("Error getting sound devices")
...
l = {'sox': SOXWrapper, 'mpg123': Mpg123Wrapper, "mplayer": MPlayerWrapper,
     "madplay": MadPlayWrapper, 'gstreamer': GStreamerBackend}

...
class SoundDeviceAlias():
    ...
    def mplayerName(self):
        ...
        return "alsa:"+ self.alsaName.replace(":","=").replace(",",".")
...
def refreshAutoGeneratedSoundAliases():
    ...
    try:
        ...
        logging.exception("Failed to search for ALSA devices")
    ...
    try:
        import jack
        client = jack.Client('PortInspectingClientKaithem')
        for i in client.get_ports(is_audio=True, is_output=False):
             p = SoundDeviceAlias()
             p.jackName = i.name
             commonSoundAliasesl[i.name] = p
        client.deactivate()
        client.close()
    except:
        logging.exception("Failed to search for JACK devices")
    ...
    class MPlayerSoundContainer(object):
        def __init__(self,filename,vol,start,end,extras,**kw):
            ...
            if "output" in kw and kw['output']:
                ...
                try:
                    getAvailableCards()
                    x = commonSoundAliases[x].mplayerName or x
                except:
                    pass

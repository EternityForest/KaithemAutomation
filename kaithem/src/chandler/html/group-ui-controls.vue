...
<template id="group-ui">
  <div>
    ...
    <div class="noselect flex-row gaps" v-if="groupData.displayTags.length > 0">
      <div
        :style="{ 'min-width': v[2].width + 'rem' }"
        v-for="v in groupData.displayTags">
        <label
          ><b>{{ v[0] }}</b></label
        >
        ...
        <template v-if="v[2].type == 'meter'">
          <div>
            <meter
              v-if="groupData.displayTagMeta[v[1]]"
              :min="groupData.displayTagMeta[v[1]].min"
              :max="groupData.displayTagMeta[v[1]].max"
              :high="groupData.displayTagMeta[v[1]].hi"
              :lo="groupData.displayTagMeta[v[1]].lo"
              :value="groupData.displayTagValues[v[1]]"></meter>
        ...
        <template v-if="v[2].type == 'led'">
          <div style="min-width: 4em">
            <span class="numval"
              ><small>{{
                groupData.displayTagValues[v[1]].toFixed(1)
              }}</small></span
            ><input
              type="checkbox"
              :class="{
                'led': 1,
                'led-red': v[2].color == 'red',
                'led-yellow': v[2].color == 'yellow',
                'led-green': v[2].color == 'green',
                'led-blue': v[2].color == 'blue',
                'led-purple': v[2].color == 'purple',
              }"
              v-bind:checked="groupData.displayTagValues[v[1]]"
              disabled />
          </div>
        </template>
        ...
        <template v-if="v[2].type == 'string_input'">
          <div>
            <input
              type="text"
              v-model="groupData.displayTagValues[v[1]]"
              v-on:change="
                setTagInputValue(
                  groupData.id,
                  v[1],

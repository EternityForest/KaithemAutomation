...
<template>
    <div class="window w-full modal" popover id="cueLogicDialog" ontoggle="globalThis.handleDialogState(event)"
        v-if="currentcue && editinggroup">
        <header>
            <div class="tool-bar">
                <h4>{{ currentcue.name }} Logic</h4>
                <button class="nogrow" data-testid="close-logic" type="button" popovertarget="cueLogicDialog"
        ...
        <script-editor :completers="completers" v-bind:example_events="example_events" v-model="currentcue.rules"
            @update:model-value="setcueproperty(currentcue.id, 'rules', $event);" v-bind:commands="availablecommands"
            v-bind:groups="cueNamesByGroupName()" v-bind:vars="editinggroup.vars" v-bind:parentgroup="editinggroup.name"
            :disabled="no_edit">
        </script-editor>

...
<template id="template">
    ...
    <div id="app" v-cloak style="display:flex; flex-wrap:wrap; justify-content: center;">
        <div class="flex-row w-full">
            ...
            <div class="window paper cols-10 grow margin-top" style="overflow: auto; flex-grow:1; min-width: 18em">
                <header>
                    <div class="tool-bar">
                        <button id="toolbar-clock">{{ clock }}</button>
                        <a class="button" :href="'/chandler/editor/' + boardname"><i class="mdi mdi-pencil"></i></a>

                        <button type="button" @click="showUtils = !showUtils"><i
                                class="mdi mdi-calculator"></i></button>
                        <input size=8
                        ...
                        <article v-if="i" style="position:relative"
                            ...
                            <header class="flex-row gaps">
                                <h3><button :disabled="i[1].utility" v-bind:class="{ highlight: i[0] == groupname }"
                                        popovertarget="groupDialog" v-on:click="selectgroup(i[1], i[0])"
                                        ...
                                        <span v-if="cuemeta[i[1].cue].inheritRules"
                                            title="This cue has rules inherited"><i
                            ...
                            <div class="flex-col nogaps scene-data-area readability-outline"
                                style="text-align: center; position:relative">
                                <img style="border:none; opacity: 0.4; height: 100%; width: 100%; object-fit: contain; position: absolute"
                                    class="v-center" onerror='this.style.display = "none"'
                                ...
                                <div style="height: 100%; width: 100%; z-index: 1">
                                    ...
                                    <div class="flex-row nogaps nogrow w-full h-min-content"
                                        style="align-items: stretch;">


                                        <div style="border-bottom-right-radius: 0.5em"
                                    ...
                                    <small v-if="i[1].active && (cuemeta[i[1].cue].next || cuemeta[i[1].cue].defaultNext)">
                                        Next:
                                        {{ (cuemeta[i[1].cue].next || cuemeta[i[1].cue].defaultNext || '').split('?')[0] }}
                                    </small>

                                    <iframe style="flex-grow:1" v-if="showPages && i[1].infoDisplay.length > 0"
                                        :src="i[1].infoDisplay"></iframe>

                                    <group-ui :unixtime="unixtime" v-bind:group-data="i[1]"
        ...
        <div popover id="groupDialog" class="window paper margin modal w-full"
            ...
            <header>
                <div class="tool-bar">
                    ...
                    <input v-if="!editingGroup.utility" type="range" style="max-width: 12rem" max=1 step=0.01 min=0
                        v-on:input="setalpha(editingGroup.id, parseFloat($event.target.value));"
                        v-on:change="setalpha(editingGroup.id, parseFloat($event.target.value));"
            ...
            <div id="cuesbox">
                <div class="flex-row align-left">
                    ...
                    <cue-iter class="w-full" v-slot="cueSlot" :cuemeta="cuemeta" :groupcues="groupcues"
                        :editingGroup="editingGroup" :groupname="groupname">
                        <button v-if="compactCues" v-on:click="jumptocue(cueSlot.i[1].id, editingGroup.id)"
                            class="compact-cuebutton"
                            v-bind:class="{ 'success': cuemeta[editingGroup.cue].name == cueSlot.i[1].name }">

                            <img style="border:none; height:100%; width:100%; position: absolute; object-fit: contain"
                                class="v-center" onerror='this.style.display = "none"' v-if="cueSlot.i[1].labelImage"
                                :src="'../WebMediaServer?labelImg=' + encodeURIComponent(cueSlot.i[1].id) + '&cacheid=' + encodeURIComponent(cueSlot.i[1].labelImageTime)">

                            <div>{{ cueSlot.i[1].name }}
                                <p>
                                    <span v-if="cueSlot.i[1].sound.length > 0" class="mdi mdi-music"></span>
                                    <span v-if="cueSlot.i[1].slide.length > 0"
                                        class="mdi mdi-projector-screen-outline"></span>
                                    <span v-if="!cueSlot.i[1].track" class="mdi mdi-arrow-expand-right"
                                        title="Cue Only"></span>
                                    <span v-if="cueSlot.i[1].hasLightingData || !cueSlot.i[1].track"
                                        class="mdi mdi-track-light" title="Cue Only"></span>
                                    <span v-if="cueSlot.i[1].length" class="mdi mdi-clock-outline"
                                        title="Timed Cue"></span>
                                    <span v-if="cueSlot.i[1].fadeIn" class="mdi mdi-chart-bell-curve-cumulative"
                                        title="Fade In"></span>
                                </p>
                            </div>
                        </button>
                        ...
                        <article class="w-sm-full margin flex-col" v-if="!compactCues"
                            ...
                            <header>
                                <button class="h-4rem w-full cuebutton" v-on:click="jumptocue(cueSlot.i[1].id, editingGroup.id)">
                                    ...
                                    <span v-if="cueSlot.i[1].hasLightingData || !cueSlot.i[1].track"
                                        class="mdi mdi-track-light" title="Cue Only"></span>
                                    <span v-if="cueSlot.i[1].length || cueSlot.i[1].relLength"
                                        class="mdi mdi-clock-outline" title="Timed Cue"></span>
                                    <span v-if="cueSlot.i[1].fadeIn" class="mdi mdi-chart-bell-curve-cumulative"
                                        title="Fade In"></span>
                            ...
                            <div class="flex-row nogaps nogrow h-12rem w-full cue-button-body"
                                ...
                                <div style="z-index: 1; height:2rem;"
                                    :class="{ 'success': cuemeta[editingGroup.cue].name == cueSlot.i[1].name, 'grow': 1, 'readability-outline': 1 }">
                                    <cue-countdown v-if="cuemeta[editingGroup.cue].name == cueSlot.i[1].name"
                                        :group="editingGroup" :cue="cuemeta[editingGroup.cue]"></cue-countdown>

                                    <div v-if="cueSlot.i[1].notes.length > 0" style="max-width: 10em;">
                                        {{ cueSlot.i[1].notes }}</div>

                                    <span v-if="(cueSlot.i[1].active && ('' + cueSlot.i[1].length).indexOf('@') > -1)"><i
                                            class="mdi mdi-clock-outline"></i>{{ cueSlot.i[1].cue.length.substring(1) }}</span>
                                </div>




                                <img class="v-center cue-button-album-art" onerror='this.style.display = "none"'
                                    v-if="cueSlot.i[1].sound"
                                    :src="'../WebMediaServer?albumArt=' + encodeURIComponent(cueSlot.i[1].id)">
...
appData.showUtils = Vue.ref(false)
appData.compactCues = Vue.ref(true)
appData.scratchpad = Vue.ref("Text here is NOT yet saved when page reloads.")
...
export default
    {
        ...
        components: {
            ...
            'cue-iter': window.httpVueLoader('../static/cue-iter.vue?cache_version=c6d0887e-af6b-11ef-af85-5fc2044b2ae0')

<template id="template">
    ...
    <datalist name="specialcues" id="specialcues">
        <option value="__rules__">All other cues in the group will inherit rules
        </option>
    </datalist>
    ...
    <div id="app" v-cloak class="flex-row gaps">
        ...
        <section id="optionsblock" class="multibar undecorated w-full">
            ...
            <div class="menubar tool-bar">
                <button type="button" popovertarget="presetsDialog"><i
                        class="mdi mdi-playlist-edit"></i>Presets</button>
                ...
                <button aria-label="Fullscreen" type="button" onclick="document.documentElement.requestFullscreen()"><i
        ...
        <main class="w-full flex-row">
            ...
            <section class="window margin w-sm-full flex-col gaps h-48rem" style="resize:both; padding-bottom: 1px;">
                <header>
                    ...
                    <div class="tool-bar">
                        <input type="text" v-model="groupfilter" placeholder="Search" list="tracks">
                        <button type="button" aria-label="Clear search" v-on:click="groupfilter = ''"><i
                                class="mdi mdi-backspace"></i>
                        </button>
                    ...
                    <div class="flex-col gaps">
                        ...
                        class="card group relative border"
                            ...
                            <header>
                                <div>
                                    <h4>
                                        <button type="button" class="w-full noselect"
                                            ...
                                            <i class="mdi mdi-wifi" v-if="i[1].mqttServer"
                            ...
                            <p class="warning" v-if="i[1].status">STATUS:
                                {{ i[1].status }}</p>
                            ...
                            <div class="w-full flex">
                                ...
                                <small>
                                    <span v-if="(i[1].active && ('' + cuemeta[i[1].cue].length).indexOf('@') > -1)">
                            ...
                            <div class="w-full flex" v-if="i[1].active && i[1].nextScheduledCue">
                                <span><i class="mdi mdi-calendar-clock"></i>
                                    {{ formatTime(i[1].nextScheduledCue[1]) }}<i class="mdi mdi-arrow-right"></i>
                            ...
                            <footer>
                                <span
                                    v-if="cuemeta[i[1].cue] && (cuemeta[i[1].cue].next || cuemeta[i[1].cue].defaultnext)">Next
                                    Cue:
                ...
                <footer class="padding">
                    <div class="tool-bar">
                        <input type="text" :disabled="no_edit" v-model="newgroupname" placeholder="New group name"
                            size="4">
            ...
            <section class="window margin flex-item" v-if="showevents"
                ...
                <div class="max-h-12rem scroll border" style="height:6em">
                    <div v-bind:class="{ 'evlisting': i[0] !== 'error', 'evlisting_err': i[0].includes('error') }"
                        v-for="i in evlog.filter((d) => (d[1].search(evfilt) > -1 || d[0].search(evfilt) > -1))">
                        {{ i[2] }}:
                        <b>{{ i[0] }}</b>
                        at
                        {{ i[1] }}:
                ...
                <label>Send Global Event:
                    <input type="text" :disabled="no_edit" v-model="evtosend" v-on:keydown.enter="sendev"
            ...
            <section v-if="showslideshowtelemetry" class="flex-item window paper h-24rem margin">
                ...
                <slideshow-telemetry :telemetry="slideshow_telemetry"></slideshow-telemetry>
            </section>
            ...
            <section v-if="showimportexport" class="flex-item window paper h-24rem">
                ...
                <div class="tool-bar" style="padding:0.25em; border-width:2px;">
                    ...
                    <button type="button" @click="uploadFileFromElement('psfile', 'setup')"
                        title="Upload a groups file"><i class="mdi mdi-folder-open"></i>Go</button>
                </div>
            </section>


            <fixture-presets-dialog :presets="presets" :fixture="selectingPresetFor"
                :fordestination="selectingPresetForDestination" :fixtureclasses="fixtureClasses"
                :fixturetype="lookupFixtureType(selectingPresetFor)"
                :currentvals="(cuevals[currentcueid] || {})[selectingPresetFor]" :currentcueid="currentcueid"
                :getpresetimage="getPresetImage" :no_edit="no_edit"></fixture-presets-dialog>


            <preset-editing-dialog :presets="presets" :no_edit="no_edit" :updatepreset="updatePreset"
                :getpresetimage="getPresetImage" :deletepreset="deletePreset" :copypreset="copyPreset"
            ...
            <div popover class="card modal paper flex-col" id="iframeDialog"
                style="background: var(--alt-control-bg); position: fixed; width:90vw; max-width: 48rem; height: 90vh; top:5vh; left:5vw; z-index: 100">
                <header>
                    <button @click="iframeDialog = null" popovertarget="iframeDialog" popovertargetaction="hide"
                        class="w-full nogrow">
            ...
            <cue-logic-dialog :groupname="groupname" :currentcue="currentcue" :groupmeta="groupmeta"
                :groupcues="groupcues" :editinggroup="editingGroup" :availablecommands="availableCommands"
                :availabletags="availableTags" :no_edit="no_edit" :setcueproperty="setCueProperty" :unixtime="unixtime">

            </cue-logic-dialog>

            <section popover ontoggle="globalThis.handleDialogState(event)" id="selectedGroupWindow"
                style="min-height: 90vh" class="modal window paper margin w-full"
                v-if="editingGroup && cuemeta[editingGroup.cue]">
                <header>

                    <p class="warning" v-if="editingGroup.status">STATUS:
                        {{ editingGroup.status }}</p>
                    <div class="tool-bar">
                        ...
                        <button type="button" class="nogrow" v-if="editingGroup.cuelen"
                            v-on:click="addTimeToGroup(editingGroup.id)"><i class="mdi mdi-clock"></i><i
                        ...
                        <button aria-label="Group Settings" data-testid="group-properties-button" class="nogrow"
                            type="button" popovertarget="groupPropsDialog">
                            <i class="mdi mdi-cog"></i>
                        </button>

                        <button aria-label="Delete Group" type="button" class="nogrow"
                            v-on:click="delgroup(groupname)"><i class="mdi mdi-delete"></i>
                        </button>

                        <button data-testid="close-group" aria-label="Close Group" type="button" class="nogrow"
                            @click="selectingPresetFor = null" popovertarget="selectedGroupWindow"
                ...
                <div id="cuesbox">
                    ...
                    <cue-table :cuemeta="cuemeta" :groupname="groupname" :groupcues="groupcues">
                        <template v-slot:header>
                            <tr>
                                <th title="Click the cue name to edit that cue">Name
                                </th>
                                <th class="desktop-only">Shortcut</th>
                                <th class="desktop-only">Fadein</th>
                                <th class="desktop-only" title="Length, 0 indicates until stopped">Length
                                </th>
                                <th title="The next cue, after this one ends">Next
                                </th>
                            ...
                            <tr
                                ...
                                <td class="rowname" data-label="Cue: " style=" user-select: none;"
                                    ...
                                    <div style="display: inline-block;">
                                        ...
                                        <img v-if="slotProps.i[1].labelImage.length > 0"
                                            :src="'../WebMediaServer?labelImg=' + encodeURIComponent(slotProps.i[1].id) + '&timestamp=' + encodeURIComponent(slotProps.i[1].labelImageTimestamp)"
                                            class="h-center" style="max-height: 1em">
                                ...
                                <td class="desktop-only" data-label="Shortcut:">
                                    <div class="tool-bar" style="width: 8em;">
                                        <input :disabled="no_edit" title="Shortcut code" style="max-width: 4.5em;"
                                            v-on:change="setCueProperty(slotProps.i[1].id, 'shortcut', $event.target.value)"
                                        ...
                                        <button type="button" style="opacity:0.3"
                                            ...
                                            v-on:click="setCueProperty(slotProps.i[1].id, 'shortcut', '__generate__from__number__')">GEN</button>
                                    </div>
                                </td>

                                <td class="desktop-only" data-label="Fade In:">
                                    <input :disabled="no_edit" type="number"
                                        v-on:change="setCueProperty(slotProps.i[1].id, 'fadeIn', $event.target.value)"
                                        style="width:5em" min=0 step="0.01" v-model="slotProps.i[1].fadeIn">
                                </td>

                                <td class="desktop-only" data-label="Length:">
                                    <input :disabled="no_edit"
                                        v-on:change="setCueProperty(slotProps.i[1].id, 'length', $event.target.value); debugCueLen($event.target.value)"
                                        class="w-12rem" list="lenoptions" v-model="slotProps.i[1].length">
                                </td>
                                ...
                                <td data-label="Next Cue:">
                                    <select style="width:98%; max-width: 24rem" v-model="slotProps.i[1].next"
                                        ...
                                        <option v-bind:value="slotProps.i[1].next">
                                            {{ slotProps.i[1].next }}</option>

                                        <option value="__random__">RANDOM</option>
                                        <option value="__shuffle__">SHUFFLE</option>
                                        <option value="__schedule__">Skip to
                                            Schedule</option>

                                    </select>

                                    <input :disabled="no_edit" class="w-12rem"
                                        v-if="Object.keys(groupcues[groupname]).length > 40" autocomplete=off
                                        list="cues_in_group" title="Select a cue to activate when this one ends"
                                        v-on:change="setnext(slotProps.i[1].id, $event.target.value)"
                    ...
                    <div class="tool-bar">
                        ...
                        <button type="button" v-on:click="add_cue(groupname, newcuename, currentcue)">
                ...
                <div v-if="currentcue">
                    <div class="tool-bar">
                        <h3><i class="mdi mdi-pencil nomargin nogrow"></i>Editing
                            Cue: {{ currentcue.name }}</h3>
                    ...
                    <div class="tool-bar w-full">
                        <label class="grow" for="cuenotes"><span><i class="mdi mdi-text"></i>Notes</span>
                        </label>

                        <input :disabled="no_edit" type="text" id="cuenotes"
                    ...
                    <p v-if="currentcue.name.startsWith('__')" class="highlight">
                        Cues starting with __ are special cues.
                        They are never chosen in random selections or as the default
                        next cue and may have predefined system
                        functions attatched.</p>


                    <span class="help" v-if="editingGroup.ext">This is an external
                        group that is defined somewhere in code.
                        Any
                        changes made here may be overwritten by the code at any
                        time.</span>

                    <div class="undecorated">
                        <div class="tool-bar">
                            <button type="button" v-bind:class="{ highlight: grouptab == 'cue' }"
                                v-on:click="grouptab = 'cue'"><i class="mdi mdi-cog-outline"></i>Normal
                                View</button>
                            <button type="button" v-bind:class="{ highlight: grouptab == 'channels' }"
                                data-testid="add-rm-fixtures-button" v-on:click="grouptab = 'channels'"><i
                                    class="mdi mdi-list-box-outline"></i>Add/Remove
                                Fixtures</button>
                        </div>

                        <div class="fadersbox flex-row nopadding" style="max-width: 100%; align-items: baseline;">
                            ...
                            <template v-for="(h, fname) in cuevals[currentcueid]">
                                ...
                                <article class="fixture card flex-col gaps noselect" v-if="fname[0] == '@'">
                                    <header>
                                        ...
                                        <div class="tool-bar">
                                            <button type="button" @click="showPresetDialog(fname, false)"
                                                popovertarget="presetForFixture"
                                                title="Select a preset for this fixture"
                                                data-testid="select-preset-for-fixture"><i
                                                    class="mdi mdi-playlist-play"></i></button>

                                            <button type="button" v-if="h.__length__"
                                            ...
                                            <select :disabled="no_edit" class="w-4rem nogrow" style="flex-basis: 2rem;"
                                                ...
                                                <option value="__dummy__">Save
                                                </option>
                                                <option value="">For Any</option>
                                                <option :value="'name' + fname" title="Save for use with this fixture">
                                                    For
                                                    This</option>
                                                <option :value="'name@h-fa' + lookupFixtureColorProfile(fname)"
                                    ...
                                    <details class="undecorated nopadding nomargin"
                                        ...
                                        <summary class="nomargin nopadding"><img
                                            ...
                                            <div class="preset-icon"
                                                v-if="h['__preset__'] && h['__preset__'].v && presets[h['__preset__'].v]">

                                                <img style="max-height: 4em; object-fit: cover;"
                                                    :src="'../WebMediaServer?file=' + encodeURIComponent(getPresetImage(h['__preset__'].v))">

                                                <div class="label"
                                                    :style="{ 'background-color': presets[h['__preset__'].v]?.html_color || 'transparent' }">
                                        ...
                                        <div class="scroll nomargin padding-bottom flex-row fader-box-inner">
                                            <h-fader :i="i[1]" :groupid="groupname"
                                                :chinfo="channelInfoForUniverseChannel(i[1].u, i[1].ch)"
                                                :currentcueid="currentcueid" :showdelete="grouptab == 'channels'"
                                                :fixcmd="h" v-for="i in dictView(h, [])">
                                ...
                                <div class="card margin">
                                    ...
                                    <div class="stacked-form w-20rem">
                                        ...
                                        <button type="button" :disabled="no_edit" v-on:click="addValToCue()"><i
                                                class="mdi mdi-plus"></i>Add Channel
                                            to
                                            Cue</button>
                                    </div>
                                        ...
                                        <table>
                                            <tr v-for="(v, i) in fixtureAssignments">
                                                ...
                                                <td><button type="button"
                                                        v-on:click="addThisFixToCurrentCue(i, 1, 1, 0)"><i
                                                            class="mdi mdi-plus"></i>Add</button>
                                                </td>
                    ...
                    <div>
                        ...
                        <cue-media-dialog :currentcue="currentcue" :groupname="groupname"
                            :setcueproperty="setCueProperty" :testsoundcard="testSoundCard" :editinggroup="editingGroup"
                            :soundcards="soundCards" :setgroupproperty="setGroupProperty"></cue-media-dialog>

                        <div class="window w-full modal" popover id="cueTextDialog"
                            ontoggle="globalThis.handleDialogState(event)">
                            <header>
                                <div class="tool-bar">
                                    <h4>{{ currentcue.name }} Text</h4>
                                    <button class="nogrow" type="button" popovertarget="cueTextDialog"
                                        popovertargetaction="hide">
                        ...
                        <div class="window w-full modal" popover id="cuePropsDialog"
                            ontoggle="globalThis.handleDialogState(event)">
                            <header>
                                <div class="tool-bar">
                                    <h4>{{ currentcue.name }}</h4>
                                    <button class="nogrow" type="button" popovertarget="cuePropsDialog"
                                        popovertargetaction="hide" data-testid="close-cue-props">
                            ...
                            <div>
                                <div class="flex-row gaps">
                                    <fieldset class="stacked-form w-sm-full">
                                        ...
                                        <div class="tool-bar">
                                            <button type="button"
                                                v-on:click="promptRenameCue(groupname, currentcue.name)"><i
                                        ...
                                        <label><span><i class="mdi mdi-pound-box-outline"></i>
                                            ...
                                            <input :disabled="no_edit" type="number" title="Cue number"
                                                v-on:change="setnumber(currentcueid, $event.target.value)" min=0
                                                step=0.1 v-model="currentcue.number">
                                        </label>
                                        ...
                                        <p>
                                            ...
                                            <label><input :disabled="no_edit"
                                                    ...
                                                    type="checkbox"
                                                    v-on:change="setCueProperty(currentcueid, 'reentrant', currentcue.reentrant)"
                                        ...
                                        <p>
                                            ...
                                            <datalist id="shortcuts">
                                                <option v-for="i of shortcuts" v-bind:value="i"></option>
                                            </datalist>
                                            ...
                                            <label><span><i class="mdi mdi-arrow-down-circle-outline"></i>Shortcut
                                                ...
                                                <input :disabled="no_edit"
                                                    ...
                                                    v-on:change="setCueProperty(currentcueid, 'shortcut', $event.target.value)"
                                                    v-model="currentcue.shortcut">
                                            </label>

                                            <label><span><i class="mdi mdi-arrow-right-circle-outline"></i>
                                                    Trigger Shortcut</span>
                                                <input :disabled="no_edit" type="text" list="shortcuts"
                                                    title="When entering this cue, trigger the shortcut in other groups"
                                                    size="8"
                                                    v-on:change="setCueProperty(currentcueid, 'triggerShortcut', $event.target.value)"
                                        ...
                                        <p>
                                            <label><span><i class="mdi mdi-calendar-clock"></i>
                                                    Schedule At</span>
                                                <input :disabled="no_edit" type="text"
                                                    v-on:change="setCueProperty(currentcueid, 'scheduleAt', $event.target.value)"
                                        ...
                                        <p>
                                            <label>
                                                <span><i class="mdi mdi-clock-outline"></i>
                                                    Fadein</span>
                                                <input :disabled="no_edit" type="number"
                                                    v-on:change="setCueProperty(currentcueid, 'fadeIn', $event.target.value)"
                                                    min=0 v-model="currentcue.fadeIn">
                                            </label>
                                            <label><span><i class="mdi mdi-clock-end"></i>
                                                    Length</span>
                                                <input :disabled="no_edit"
                                                    v-on:change="setCueProperty(currentcueid, 'length', $event.target.value); debugCueLen($event.target.value)"
                                        ...
                                        <p>
                                            <label><span><i class="mdi mdi-dice-multiple-outline"></i>
                                                    Vary Length</span>
                                                <input :disabled="no_edit" type="number"
                                                    v-on:change="setCueProperty(currentcueid, 'lengthRandomize', $event.target.value)"
                                                    title="Randomize the cue length +- this amount"
                                                    v-model="cuemeta[currentcueid].lengthRandomize" min=0>
                                            </label>
                                    ...
                                    <fieldset class="stacked-form w-sm-full">
                                        ...
                                        <datalist id="cues_in_group" name="cues_in_group">
                                            <option v-for="i in Object.keys(groupcues[groupname]).sort()"
                                                v-bind:value="i">
                                                {{ i }}</option>
                                        </datalist>
                                    </fieldset>
                    ...
                    <div class="window w-full modal" popover id="groupNotesDialog"
                        ontoggle="globalThis.handleDialogState(event)">
                        <header>
                            <div class="tool-bar">
                                <h4>{{ editingGroup.name }} Notes</h4>
                                <button class="nogrow" type="button" data-testid="close-group-notes"
                                    popovertarget="groupNotesDialog" popovertargetaction="hide">
                                    <i class="mdi mdi-close"></i>Close
                                </button>
                            </div>
                        </header>
                    ...
                    <div class="window w-full modal" popover id="groupPropsDialog"
                        ontoggle="globalThis.handleDialogState(event)">
                        <header>
                            <div class="tool-bar">
                                <h4>Group Settings</h4>
                                <button class="nogrow" type="button" data-testid="close-group-settings"
                                    popovertarget="groupPropsDialog" popovertargetaction="hide">
                                    <i class="mdi mdi-close"></i>Close
                                </button>
                            </div>
                        </header>
                        ...
                        <div class="flex-row gaps">
                            ...
                            <fieldset class="stacked-form w-sm-full">
                                ...
                                <label><span><i class="mdi mdi-layers-triple"></i>
                                    ...
                                    <select :disabled="no_edit"
                                        ...
                                        v-on:input="setGroupProperty(groupname, 'blend', $event.target.value)"
                                        v-model="editingGroup.blend" data-testid="group_blend_mode">
                                        ...
                                        <option
                                            ...
                                            HTP</option>
                                        <option title="Limit maximum level">inhibit
                                        </option>

                                        <option v-for="i in blendModes">{{ i }}
                                        </option>
                                ...
                                <label title="You can use this with __random__ to create a shuffle effect.">Default
                                    ...
                                    <input :disabled="no_edit" list="nextcueoptions"
                                        v-on:change="setGroupProperty(groupname, 'defaultNext', $event.target.value)"
                            ...
                            <div class="card w-sm-full">
                                ...
                                <div class="stacked-form">
                                    <label> Sound Output<input
                                            ...
                                            v-on:change="setGroupProperty(groupname, 'soundOutput', $event.target.value)"
                            ...
                            <div class="card w-sm-double">
                                ...
                                <div class="stacked-form">
                                    <p>
                                        <label>Utility Group(No controls)
                                            <input :disabled="no_edit" type="checkbox"
                                                v-on:change="setGroupProperty(groupname, 'utility', editingGroup.utility)"
                                        ...
                                        <label>Hide in Runtime Mode
                                            <input :disabled="no_edit" type="checkbox"
                                                v-on:change="setGroupProperty(groupname, 'hide', editingGroup.hide)"
                                                v-model="editingGroup.hide"
                                                title="If checked, the group is a utility group without sidebar controls. Use for embedding CCTV, or basic state machines.">
                                        </label>
                                    </p>

                                    <label title="This URL is shown in a mini window in the sidebar">Sidebar
                                        info URL
                                        <input :disabled="no_edit"
                                ...
                                <details class="help">
                                    ...
                                    <p>Tags will be created if they do not exist.
                                        Use the tag settings page to configure
                                        defaults an limits</p>

                                    <p>Expression tags are allowed, =tv('tag1') + 6
                                        will display 6 plus the tag 'tv'</p>

                                    <p>You can also add inputs to the group
                                        overview. You can then respond to these
                                        inputs
                                        in the cue logic.
                                    </p>
                                ...
                                <table border="1" class="w-full">
                                    ...
                                    <tr v-for="(v, i) in editingGroup.displayTags">
                                        <td><input :disabled="no_edit" v-model="v[0]" class="w-6rem"
                                                ...
                                                v-on:change="setGroupProperty(groupname, 'displayTags', editingGroup.displayTags);">
                                        </td>
                                        ...
                                        <td><input :disabled="no_edit" type="text" list="tagslisting" v-model="v[1]"
                                        ...
                                        <td><select v-model="v[2].type" class="w-6rem" data-testid="display_tag_type"
                                                ...
                                                <option value="string_input">Text
                                                    Input</option>
                                                <option value="numeric_input">
                                                    Numeric Input</option>
                                                <option value="switch_input">Switch
                                                    Input</option>
                                                <option value="null">Don't Show
                                                </option>
                                            ...
                                            <template v-if="v[2].type == 'led'">
                                                <select :disabled="no_edit" v-model="v[2].color"
                                                    ...
                                                    <option value="yellow">Yel
                                                    </option>
                                                    <option value="green">Grn
                                                    </option>
                                        ...
                                        <td><button data-testid="display_tag_delete"
                                                v-on:click="editingGroup.displayTags.splice(i, 1); setGroupProperty(groupname, 'displayTags', editingGroup.displayTags);">Delete</button>
                                ...
                                <div class="stacked-form">
                                    <label>Download just this group
                                        <a class="button"
                                            ...
                                            title="Download your group as a file"><i
                                    ...
                                    <p>This playlist should work in other apps such
                                        as VLC on other computers, as long as
                                        the
                                        same media folders are
                                        in the same place relative to your home
                                        folder.
                                    </p>

                                    <label>Add new cues from playlist
                                        <a class="button" v-bind:href="'uploadm3u?id=' + groupname"
                                            title="Upload a playlist"><i class="mdi mdi-folder-open"></i>Upload
                                            M3U
                                            Playlist</a>
                                    </label>

                                    <p>Uploads use fuzzy search and should work as
                                        long as media files exist somewhere in a
                                        media folder, you can use playlists from
                                        other devices.
                                    </p>
                            ...
                            <div class="card w-sm-full" v-if="editingGroup.blend != 'normal'">
                                ...
                                <div class="stacked-form">
                                    ...
                                    <label>Type
                                        <select v-model="evtypetosend">
                                            <option value="int">Integer</option>
                                            <option value="float">Real Number
                                            </option>
                                            <option value="str">Text</option>
                                        </select>
                                    </label>
                            ...
                            <div class="card w-sm-full" v-if="editingGroup.blend != 'normal'">
                                ...
                                <div class="stacked-form">
                                    ...
                                    <p>
                                        <label v-for="k, v of dictView(editingGroup.blendArgs, [])">{{ k[0] }}:
                                            <input :disabled="no_edit" type="number" style="width:5em" step=0.01
                                                v-bind:title="k[0]"
                                                v-on:change="setGroupProperty(groupname, 'blendArgs', editingGroup.blendArgs)"
                            ...
                            <div class="card w-sm-double">
                                ...
                                <p class="help">You have to refresh the player for
                                ...
                                <details>
                                    ...
                                    <dl>
                                        ...
                                        <dd>Empty if cue has no length, otherwise,
                                            00:00:00 formatted countdown to end
                                            of current cue.
                                        </dd>
                            ...
                            <div class="card w-sm-full">
                                ...
                                <div class="stacked-form">
                                    ...
                                    <label>Perspective Distance
                                        <span><input type="range" min="0" max="500" :disabled="no_edit"
                                                v-model="editingGroup.slideshowTransform.perspective"
                                                @change="setGroupProperty(editingGroup.id, 'slideshowTransform', editingGroup.slideshowTransform)"
                                                @input="setGroupProperty(editingGroup.id, 'slideshowTransform', editingGroup.slideshowTransform)">
                                    ...
                                    <label>Scale X
                                        <span><input type="range" min="0" max="100" :disabled="no_edit"
                                    ...
                                    <label>Keystone X
                                        <span><input type="range" min="-180" max="180" :disabled="no_edit"
                            ...
                            <div class="card w-sm-full">
                                ...
                                <p>Cue providers let you automatically import cues
                                    from a folder of media files.</p>
                            ...
                            <div>
                                ...
                                <table border="1" v-if="groupmeta[groupname].history">
                                    ...
                                    <tr v-for="v, i of groupmeta[groupname].history">
                                        <td>{{ v[0] }}</td>
                                        <td>{{ (new Date(v[1] * 1000)).toLocaleString() }}
                                        </td>
                                    </tr>
...
<script type="module">
import { appData, appMethods, appComputed } from './console.mjs?cache_version=c6d0887e-af6b-11ef-af85-5fc2044b2ae0';

...
<template id="template">
    ...
    <datalist name="colorprofiles" id="colorprofiles">
        ...
        <option value="rgb.generic.2010">Typical common RGB LED fixture circa 2010-2020</option>
        <option value="rgbw.generic.2010">Typical common RGBW LED fixture circa 2010-2020</option>

        <option value="rgb.generic.2020">Typical common RGB LED fixture circa 2020-2030</option>
        <option value="rgbw.generic.2020">Typical common RGBW LED fixture circa 2020-2030</option>
    </datalist>
    ...
    <div id="app" v-cloak class="flex-row gaps">
        ...
        <main class="w-full flex-row">
            ...
            <section v-if="showFixtureSetup" class="flex-item window margin min-h-36rem w-sm-full"
                ...
                <select data-testid="fixture-type-to-edit" v-model="selectedFixtureClass"
                    v-on:change="getfixtureclass(selectedFixtureClass)">
                    <option v-for="i in Object.keys(fixtureClasses)" :value="i">{{ i }}</option>
                </select>
                ...
                <div v-if="selectedFixtureClass" class="margin">
                    ...
                    <div class="stacked-form">
                        <p class="help">
                            ...
                            so that they can share all presets called "name@profile". A preset for "rgb" will
                            show up for any fixture where the profile *starts with* "rgb", including
                            "rgbw", "rgb.generic", etc.
                        </p>
                        <label>
                            Color Profile:
                            <input list="colorprofiles" v-on:change="pushfixture(selectedFixtureClass)"
                    ...
                    <div v-for="(v, i) in fixtureClasses[selectedFixtureClass].channels || []">
                        ...
                        <label v-if="fixtureClasses[selectedFixtureClass].channels[i].type == 'fine'">
                            Matching Coarse:
                            <input v-on:change="pushfixture(selectedFixtureClass)"
                                title="The corresponding coarse channel for this fine channel" min=0 max=64
                                type="number" v-model="fixtureClasses[selectedFixtureClass].channels[i].coarse">
                        </label>

                        <label v-if="fixtureClasses[selectedFixtureClass].channels[i].type == 'fixed'">
                            Fixed Value:
                            <input v-on:change="pushfixture(selectedFixtureClass)" title="The fixed DMX channel value"
                                min=0 max=256 type="number"
                                v-model="fixtureClasses[selectedFixtureClass].channels[i].value">
                        </label>
            ...
            <section v-if="showfixtureassg" class="flex-item window paper">
                ...
                <details class="help">
                    ...
                    <summary><i class="mdi mdi-help-circle-outline"></i></summary>Here is where you actually assign
                    fixtures
                    to
                    channels after creating the desired fixture types.
                    Clicking Update will cause the new assignments to take effect immediately. Use the save settings
                    button
                    to
                    make them permanent.
                </details>
                ...
                <table border="1">
                    ...
                    <tr v-for="i in dictView(fixtureAssignments, ['universe', 'channel'])">
                        <td>{{ i[1].name }}</td>
                        <td>{{ i[1].type }}</td>
                        <td><input class="w-6rem" v-on:change="setFixtureAssignment(i[1].name, i[1])"
                                v-model="i[1].universe"></td>
                        <td><input class="w-4rem" v-on:change="setFixtureAssignment(i[1].name, i[1])"
                                v-model="i[1].addr"></td>

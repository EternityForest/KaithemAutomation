...
def rawlistsend(subject, message, list):
    ...
    raw_send(message, l, subject, registry.get(
        'system/mail/lists')[list]['name'])
...
def check_credentials():
    ...
    try:
        ...
        messagebus.postMessage("/system/notifications/errors",
                               "Bad username or password for system email account\n"+repr(e))
    except socket.gaierror:
        messagebus.postMessage("/system/errors/mail/other", repr(e))
        messagebus.postMessage(
            "/system/notifications/errors", "Unkown mail host\n"+repr(e))
    finally:
        try:
            server.quit()
        except:

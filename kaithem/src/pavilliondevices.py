...
class RemoteSquirrelProgram():
    ...
    def getPreprocessedCode(self, code=None, minify=True):
        ...
        x = {"kaithem": kaithemobj.kaithem,
             "module": modules_state.scopes[self.module]}
        code = mako.template.Template(
            code, uri="SquirrelTemplate"+self.module+'_'+self.resource, global_vars=x)
        code = code.render(**x)
        if minify:
            code = sqminify(code)
        code = "//"+(base64.b64encode(hashlib.sha256(code.encode("utf8")
                                                     ).digest()).decode("utf8"))[:14]+"\n" + code
        return code
...
class PavillionDevice(Device):
    ...
    def handleIncomingTagValue(self, value, good_ts, timestamp, name, tag, raw_timestamp, server):
        ...
        if good_ts:
            if ((timestamp > tag.timestamp) and (tag.currentSource == "shared")) or not (tag.remote_writable):
                ...
                self.pclient.sendMessage(
                    "core.tagv", name, struct.pack("<fq", tag.value, int(t)))
    ...
    def __init__(self, name, data):
        ...
        self.lowSignalAlert = alerts.Alert(
            "/devices/"+name+".lowsignalalert", tripDelay=80, autoAck=True)
        self.unreachableAlert = alerts.Alert(
            "/devices/"+name+".lowsignalalert", tripDelay=5, autoAck=True)
            ...
            def onServerStatusUpdate(self, server):
                ...
                if not server.batteryState() == 'unknown':
                    ...
                    if not DeviceObject.batteryStatusTag:
                        ...
                        DeviceObject.batteryStatusTag = tagpoints.Tag(
                    ...
                    if not DeviceObject.batteryStatusClaim:
                        DeviceObject.batteryStatusClaim = DeviceObject.batteryStatusTag.claim(
                            blevel, "reported", 51)
        ...
        self.pclient = Client2(self.onPavillionConnect, clientID=self.cid,
                               psk=self.psk, address=(data['address'], data['port']))
        ...
        def genericMessage(target, name, data, source):
            try:
                kaithemobj.kaithem.message.post(
                    "/devices/"+self.name+"/msg/"+target, (name, data, source))
                if target == "core.print":
                    self.messages.append(
                        time.time(), name, data.decode('utf8'), source)
                    ...
                    self.handleIncomingTagValue(
                        value, good_ts, localized_timestamp, name, self.tagPoints[name], timestamp, server)

                elif target == "core.tag":
                    # These messages contain pretty much the complete tag state,
                    # We use them to init new tags
                    value, min, max, interval, flags, raw_timestamp = struct.unpack(
                        "<ffffBq", data)
                    ...
                    with self.lock:
                        ...
                        if not name in self.tagClaims:
                            self.tagClaims[name] = t[name].claim(
                                value, "shared", 51, ts, "DoNotSend")
                            ...
                            if t[name].remote_writable:
                                def tagHandler(value, timestamp, annotation):
                                    ...
                                    try:
                                        ...
                                        t = int(self.pclient.getServer(
                                        ).toRemoteMonotonic(timestamp)*10**6)
                                    except:
                                        # Awful hack just in case our time is not synced.
                                        t = t[name].lastRemoteTs+1
                                        t[name].lastRemoteTs += 1
                                    self.pclient.sendMessage(
                                        "core.tagv", name, struct.pack("<fq", value, int(t)))
                        ...
                        # At who has newer data, even though bidirectional tags aren't exact.
                            ...
                            server = self.pclient.getServer(source)
                            self.handleIncomingTagValue(
                                value, good_ts, ts, name, t[name], raw_timestamp, server)
                ...
                if self.lastError < (time.time() - 10*60):
                    syslogger.exception(
                        "Error handling message from client(Log ratelimit: 10min)")
                else:
        ...
        with self.lock:
            ...
            try:
                ...
                while p:
                    ...
                    p = p[1024:]
                    c.call(4098, struct.pack("<L", pos) +
                           name.encode("utf8")+b"\x00"+x)
                    pos += len(x)
...
def updateProgram(module, resource, data, upload=True):
    with lock:
        ...
        if (module, resource) in loadedSquirrelPrograms:
            loadedSquirrelPrograms[module, resource].unload()
        loadedSquirrelPrograms[module, resource] = RemoteSquirrelProgram(
        ...
        if upload:
            ...
            def f():
                try:
                    ...
                    try:
                        loadedSquirrelPrograms[module, resource].errors.append(
                            [time.time(), traceback.format_exc()])
                    except:
                        pass
                    syslogger.exception(
                        "Could not upload program: " + module+"."+resource)
            workers.do(f)

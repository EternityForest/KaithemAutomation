...
class WebInterface():
    ...
    def deletemoduletarget(self, **kwargs):
        pages.require("/admin/modules.edit")
        pages.postOnly()
        rmModule(kwargs['name'], "Module Deleted by " +
                 pages.getAcessingUser())
        messagebus.postMessage("/system/notifications", "User " +
                               pages.getAcessingUser() + " Deleted module " + kwargs['name'])
    ...
    def module(self, module, *path, **kwargs):
        ...
        if not path:
            ...
            if path[0] == 'getfileresource':
                ...
                if os.path.isfile(dataname):
                    return serve_file(dataname,
                                      content_type=mimetypes.guess_type(path[1], False)[
                                          0] or "application/x-unknown",
                                      disposition="inline;",
                                      name=path[1])
            ...
            if path[0] == 'deleteresourcetarget':
                ...
                pages.postOnly()
                rmResource(
                    module, kwargs['name'], "Resource Deleted by " + pages.getAcessingUser())

                messagebus.postMessage("/system/notifications", "User " + pages.getAcessingUser() + " deleted resource " +
                                       kwargs['name'] + " from module " + module)
                messagebus.postMessage("/system/modules/deletedresource", {
                                       'ip': cherrypy.request.remote.ip, 'user': pages.getAcessingUser(), 'module': module, 'resource': kwargs['name']})
                if len(util.split_escape(kwargs['name'], '/', '\\')) > 1:
                    raise cherrypy.HTTPRedirect('/modules/module/'+util.url(
                        module)+'/resource/'+util.url(util.module_onelevelup(kwargs['name'])))
                else:
                    raise cherrypy.HTTPRedirect(
...
def resourceUpdateTarget(module, resource, kwargs):
    ...
    with modules_state.modulesLock:
        ...
        if t == 'permission':
            ...
            resourceobj['allow-origins'] = [i.strip()
                                            for i in kwargs['allow-origins'].split(',')]
            ...
            if 'enable' in kwargs:
                try:
                    ...
                    # Remove the old event even before we even do a test run of setup. If we can't do the new version just put the old one back.
    ...
    if len(x) > 1:
        raise cherrypy.HTTPRedirect("/modules/module/"+util.url(module) +
                                    '/resource/'+'/'.join([util.url(i) for i in x[:-1]])+"#resources")
    else:

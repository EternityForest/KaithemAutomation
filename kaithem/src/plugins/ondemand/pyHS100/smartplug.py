...
class SmartPlug(SmartDevice):
    ...
    def turn_on(self):
        ...
        #Kaithem Customizations, notice we only ratelimit turning on, not off
        
        #If it's been over a second since the last transition attempt, then
        #We decrease the delay time. otherwise we increase it exponentially.
        
        if time.time()-self.lastTransition> 1:
            self.currentDelay= max(0.05, self.currentDelay/2)
        else:

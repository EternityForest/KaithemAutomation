...
class KasaDevice(devices.Device):
    def __init__(self,name,data):
        ...
        if data.get('temp.ssid',''):
            getDevice(data['device.locator']).set_wifi(data.get('temp.ssid'), data.get('temp.psk'))
        ...
        with self.lock:
            ...
            try:
                if state:
                    ...
                    getDevice(self.data.get("device.locator")).turn_on()
                else:
                    getDevice(self.data.get("device.locator")).turn_off()
            except:
                if self.lastLoggedUnreachable< time.monotonic()-30:
                    self.handleError("Device was unreachable")
                    self.lastLoggedUnreachable=time.monotonic()
    ...
    def getSwitch(self,channel):
        with self.lock:
            ...
            if channel>0:
                raise ValueError("This is a 1 channel device")
            try:
                s = getDevice(self.data.get("device.locator")).state=="ON"
            except:
                if self.lastLoggedUnreachable< time.monotonic()-30:
                    self.handleError("Device was unreachable")
                    self.lastLoggedUnreachable=time.monotonic()
...
class KasaBulb(KasaDevice):
    ...
    def setSwitch(self,channel, state,duration=1):
        ...
        with self.lock:
            ...
            try:
                if state:
                    getDevice(self.data.get("device.locator")).turn_on(duration)
                    self.wasOff=False
                else:
                    getDevice(self.data.get("device.locator")).turn_off(duration)
                    self.wasOff=True

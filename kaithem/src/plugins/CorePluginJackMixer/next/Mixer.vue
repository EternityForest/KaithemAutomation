  ...
  <div v-cloak>
    ...
    <section class="window">
      <header>
        ...
        <datalist id="inports">
          <option v-for="i in sortedConnectables(inports)" :key="i">{{ i }}</option>
        </datalist>

        <datalist id="outports">
          <option v-for="i in sortedConnectables(outports)" :key="i">{{ i }}</option>
        ...
        <div class="tool-bar">
          <button :class="{ bold: activePane === 'mixer' }" @click="activePane = 'mixer'">
            Mixer ({{ boardResource }})
          </button>
          <button :class="{ bold: activePane === 'presets' }" @click="activePane = 'presets'">
            Presets/Defaults
          </button>
          <button :class="{ bold: activePane === 'status' }" @click="activePane = 'status'">
            Status
          </button>
          ...
          <p>Preset:</p>
          <input list="presets" v-model="loadedPreset" placeholder="Preset name" />
      ...
      <div v-if="activePane === 'presets'">
        ...
        <details class="help">
          ...
          Presets let you load and save the entire state of the mixing board. When kaithem boots,
          the preset named "default" will be loaded if it exists. Presets are saved immediately to
      ...
      <div class="flex" v-if="activePane === 'mixer' && ui_ready">
        ...
        >
          <header>
            <div v-if="channel.type === 'audio' || !channel.type">
              ...
              <div class="menubar tool-bar">
                <h3 style="display: inline-block">{{ channelname }}({{ channel.channels }})</h3>
                <p
                  :class="{
                ...
                >
                  <small data-testid="channel-status">{{ channelStatus[channelname] }}</small>
                </p>
                ...
                <button data-testid="delete-button" @click="confirmDelete(channelname)">
          ...
          <div v-if="showLabels">
            <label>
              ...
              <input
                @change="api.send(['set_label_image', channelname, ($event.target as HTMLInputElement).value])"
          ...
          <details>
            ...
            <div class="tool-bar">
              <label>
                In:<input
                  ...
                  @change="setInput(channelname, ($event.target as HTMLInputElement).value)"
                />
              </label>
            </div>
            ...
            <div class="chain scroll h-24rem noselect" data-testid="effect-chain">
                ...
                <details class="undecorated">
                  <summary data-testid="effect-title-id">
                    <div class="menubar tool-bar inline" style="width: 85%">
                      ...
                      <button data-testid="move-effect-up-button" @click="moveEffectUp(channelname, index)">
                  ...
                  <table class="col-12 h-center" border="1">
                    <tr
                      v-for="param in sortedParams(effect.params)"
                      :key="param.name"
                      class="param"
                    ...
                    >
                      <template v-if="param.type === 'float'">
                        ...
                        <td>
                          <SmoothRange
                            ...
                            :max="param.max"
                            :step="param.step"
                            @input="setParam(channelname, effect.id, param.name, parseFloat(($event.target as HTMLInputElement).value))"
                          />
                        </td>
                      ...
                      <template v-if="param.type === 'int'">
                        ...
                        <td>
                          <SmoothRange
                            ...
                            :max="param.max"
                            :step="param.step"
                            @input="setParam(channelname, effect.id, param.name, parseInt(($event.target as HTMLInputElement).value))"
                          />
                        </td>
                      ...
                      <template v-if="param.type === 'string.int'">
                        ...
                        <td>
                          <input
                            ...
                            :max="param.max"
                            :step="param.step"
                            @change="setParam(channelname, effect.id, param.name, parseInt(($event.target as HTMLInputElement).value))"
                          />
                        </td>
                      ...
                      <template v-if="param.type === 'bool'">
                        ...
                        <td>
                          <input
                            type="checkbox"
                            v-model="param.value"
                            @input="setParam(channelname, effect.id, param.name, ($event.target as HTMLInputElement).checked)"
                          />
                        </td>
                        <td>{{ param.value }}</td>
                      </template>
                      ...
                      <template v-if="param.type === 'JackInput'">
                        ...
                        <td>
                          <input
                            list="inports"
                            v-model="param.value"
                            @change="setParam(channelname, effect.id, param.name, ($event.target as HTMLInputElement).value)"
                          />
                        </td>
                        <td>{{ param.value }}</td>
                      </template>
                      ...
                      <template v-if="param.type === 'enum'">
                        <td>{{ param.displayName }}</td>
                        <td>
                          <select v-model="param.value" @input="setParam(channelname, effect.id, param.name, ($event.target as HTMLSelectElement).value)">
                            <option v-for="i of param.options" :key="i[1]" :value="i[1]">
                              {{ i[0] }}
                            </option>
                          </select>
                        </td>
              ...
              <div v-if="showAddEffect" class="effect" data-testid="effects-menu">
                <h3>
                  Add Effect<button @click="showAddEffect = false">Hide</button>
                </h3>
                <div class="tool-bar">
                  <input v-model="fxSearch" placeholder="Search" />
                  <button @click="fxSearch = ''"><i class="mdi mdi-backspace"></i></button>
                </div>
                <div style="height: 12em; overflow: scroll">
                  <template v-for="i in effectTypes" :key="i.type">
                    <div
                      v-if="canUseEffect(i, channel.channels) && (i.displayType + i.help).toLowerCase().includes(fxSearch)"
                    >
                      <h4>{{ i.displayType }}</h4>
                      <p>{{ i.help }}</p>
                      <button :data-testid="'add-effect-' + i.type" @click="api.send(['addEffect', channelname, i.type])">
                        Add
                      </button>
                    </div>
                  </template>
            ...
            <div class="tool-bar">
              ...
              <input
                ...
                list="inports"
                @change="setOutput(channelname, ($event.target as HTMLInputElement).value)"
            ...
            <div class="tool-bar">
              ...
              <input
                ...
                @change="setFuse(channelname, ($event.target as HTMLInputElement).value)"
              />
              <p>dB</p>
            </div>
          </details>
          ...
          <footer class="padding noselect">
            ...
            <div class="tool-bar">
              <SmoothRange
                ...
                @input="setFader(channelname, parseFloat(($event.target as HTMLInputElement).value))"
                step="0.5"
                min="-60"
                max="20"
                :title="channel.fader"
              ...
              <button @click="setMute(channelname, !channel.mute)" title="Mute channel" type="checkbox">
                <i
                  v-if="channel.mute"
            ...
            <div class="tool-bar">
              <meter min="-70" max="10" high="-5" data-testid="channel-level-meter" :value="channel.level"></meter>
        ...
        <div class="flex-col gaps inline w-sm-full">
          <article class="channel window h-12rem flex-col margin w-sm-full">
            ...
            <h4>New Channel</h4>
            <div class="tool-bar">
              <input v-model="newchannelname" style="max-width: 8em" data-testid="new-channel-name" />
              <button data-testid="add-mono-channel" @click="api.send(['addChannel', newchannelname, 1])">
                Add Mono
              </button>
              <button data-testid="add-stereo-channel" @click="api.send(['addChannel', newchannelname, 2])">
                Add Stereo
              </button>
            </div>
            <footer></footer>
          ...
          <article class="channel window margin w-sm-full">
            ...
            <details class="help">
              <summary><i class="mdi mdi-help-circle-outline"></i></summary>
              Route your audio to the input(Named {{ recorderPortName }}) to use this.

              <br />
              Files in
              <a href="/settings/files/${os.path.join(directories.vardir,'recordings','mixer')|u}">
                VARDIR/recordings/mixer
              </a>
            </details>
            <div class="tool-bar margin">
              <p>Channels:</p>
              <input aria-label="Channels" type="number" v-model="recordChannels" style="max-width: 3em" />
              <p>File Prefix:</p>
              <input v-model="recordName" size="1" />
            </div>
            <div class="tool-bar margin">
              <button @click="api.send(['record', recordName, recordChannels])">Start</button>
...
import SmoothRange from './components/SmoothRange.vue';
...
const inports = ref<Record<string, any>>({});
const outports = ref<Record<string, any>>({});
...
const midiinports = ref<Record<string, any>>({});
const midioutports = ref<Record<string, any>>({});
const recorderPortName = ref(properties.boardname.replace(':', '_').replace('/', '_'));
const recordName = ref(properties.boardname.replace(':', '_').replace('/', '_') + '_');
...
function compareParameters(a: any, b: any) {
...
function sortedParams(pl: Record<string, any>) {
  const l = [];
  for (const i in pl) {
    pl[i].name = i;
    l.push(pl[i]);
...
function getExcalidrawLink(channel: string) {
  return (
    ...
    '&resource=' +
    encodeURIComponent('media/mixers/sketches/channel_' + properties.boardname.split(':')[1] + '.excalidraw.png') +
    '&callback=' +
    encodeURIComponent(`/settings/mixer/${properties.boardname}/${channel}/set_channel_img`) +
    '&ratio_guide=16_9'
  );
}
...
function confirmSavePreset(c: string) {
  if (globalThis.confirm("Do you really want to save this preset? Anything named 'default' will be loaded at startup.")) {
...
function setParam(channel: string, effect: string, parameter: string, value: any) {

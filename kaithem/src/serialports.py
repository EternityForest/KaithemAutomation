...
def manage():
    with lock:
        portInterfaces = {i: portInterfaces[i]
                          for i in portInterfaces if portInterfaces[i]()}
...
class PortInterface():
    def __init__(self, port, alertPriority="warning", alertZone="", *, **kwargs):
        with lock:
            ...
            self.portSettings = kwargs.update(
                {'baudrate': 9600, 'timeout': 0.1, 'write_timeout': 10, 'inter_byte_timeout': 0.03})
            self.alert = alerts.Alert(
                "/serial/ports/"+port+"/disconnected", priority=alertPriority, zone=alertZone)

...
def tags_docstring():
    ...
    TagPoints are threadsafe containers for variables that provide easy debugging and subscription to changes.
    ...
    When a tag is first created, it's value is the default for that type, and it's timestamp will be 0.
    ...
    This of course, means that it is possible to create deadlocks if you do something really crazy.

    To prevent this, tags have a timeout, of around ten seconds,
    after which they will give up on most actions and raise a RuntimeError. 
    ...
    ### Expression Tags
    Any tag having a name that begins with an equals sign will be 
    created with a getter that evaluates the name as an expression.  
    The priority of the expression source will be 98.

    You have access to time, math, random, re, 
    plus the tag itself as 'tag', and anything else you put in tag.eval_context in code.


    #### the tv(n) function

    The tv function is used from within a tag expression. 
    It returns the value of a numeric tag with the given name, 
    and adds the tag to the list of source tags.
    ...
    The way this works is that tag.data_source_widget is created, a Widgets.DataSource instance having id "tag:TAGNAME", with the given permissions.
